"""
Deep Learning Inference for Home Insurance Lead Scoring
...
class HomeInsuranceNeuralNetwork(nn.Module):
    """Neural Network for Home Insurance - must match training architecture"""
...
class HomeInsuranceDeepLearningScorer:
    """Deep Learning Scorer for Home Insurance Leads"""
    
    def __init__(self, model_dir='backend/models/home_insurance_scoring/saved_models'):
    ...
    def load_model(self):
        ...
        try:
            ...
            self.feature_columns = joblib.load(f'{self.model_dir}/home_insurance_dl_features.pkl')
            
            # Load scaler and encoders
            self.scaler = joblib.load(f'{self.model_dir}/home_insurance_dl_scaler.pkl')
            self.label_encoders = joblib.load(f'{self.model_dir}/home_insurance_dl_label_encoders.pkl')
            ...
            self.model.load_state_dict(torch.load(
                f'{self.model_dir}/home_insurance_dl_model.pth',
                map_location=self.device
            ))
            self.model.eval()
            
            logger.info("Home Insurance Deep Learning model loaded successfully")
            
        except Exception as e:
            logger.error(f"Error loading Home Insurance Deep Learning model: {e}")
            raise
    ...
    def score_lead(self, lead_data: Dict) -> Dict[str, Any]:
        """Score a single home insurance lead"""
        try:
            # Preprocess
...
if __name__ == "__main__":
    # Test the scorer
    scorer = HomeInsuranceDeepLearningScorer()

    test_lead = {
        'lead_id': 'HOME_DL_TEST_001',
        'age': 42,
        'income': 85000,
        'policy_type': 'home',
    ...
    print(f"Home Insurance Deep Learning Score: {result}")
