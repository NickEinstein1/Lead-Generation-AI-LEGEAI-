...
class PipelineOrchestrator:
    ...
    async def _handle_lead_created(self, event_data: Dict[str, Any]):
        ...
        try:
            ...
            await integration_hub.queue_sync_operation({
                'type': 'crm_sync',
                'operation_id': f"crm_sync_{datetime.now(timezone.utc).timestamp()}",
                'data': event_data,
                'direction': 'outbound'
            })
    ...
    async def _handle_lead_updated(self, event_data: Dict[str, Any]):
        ...
        try:
            ...
            await integration_hub.queue_sync_operation({
                'type': 'crm_sync',
                'operation_id': f"crm_update_{datetime.now(timezone.utc).timestamp()}",
                'data': event_data,
                'direction': 'outbound'
            })
    ...
    async def get_system_health(self) -> Dict[str, Any]:
        ...
        health_report = {
            ...
            'uptime': (datetime.now(timezone.utc) - self.metrics.last_updated).total_seconds(),
            'metrics': {
