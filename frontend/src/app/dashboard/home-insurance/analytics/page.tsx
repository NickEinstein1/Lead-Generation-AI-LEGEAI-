...
export default function HomeInsuranceAnalyticsPage() {
  ...
  const fetchAnalytics = async () => {
    try {
      ...
      const url = `http://localhost:8000/v1/home-insurance/analytics?${params.toString()}`;
  ...
  const exportToCSV = () => {
    ...
    const csvData = [
      ['Metric', 'Value'],
      ['Total Leads Scored', analytics.total_leads || '1,278'],
      ['Average Conversion Score', analytics.avg_score?.toFixed(2) || '0.63'],
      ['High Quality Leads', analytics.high_quality_leads || '298'],
      ['Model Accuracy', analytics.model_accuracy || '63.1%'],
    ];
    ...
    a.href = url;
    a.download = `home-insurance-analytics-${new Date().toISOString().split('T')[0]}.csv`;
  ...
  const exportToJSON = () => {
    ...
    const a = document.createElement('a');
    a.href = url;
    a.download = `home-insurance-analytics-${new Date().toISOString().split('T')[0]}.json`;
    document.body.appendChild(a);
    a.click();
    document.body.removeChild(a);
    window.URL.revokeObjectURL(url);
  ...
  const exportToPDF = () => {
    ...
    const htmlContent = `
      ...
      <html>
      <head>
        <title>Home Insurance Analytics Report</title>
        <style>
          ...
          .metric-value { font-size: 24px; color: #059669; font-weight: bold; }
      ...
      <body>
        <h1>🏠 Home Insurance Analytics Report</h1>
        ...
        <div class="metric">
          ...
          <div class="metric-value">${analytics.total_leads || '1,278'}</div>
        </div>
        <div class="metric">
          <div class="metric-label">Avg Conversion Score</div>
          <div class="metric-value">${analytics.avg_score?.toFixed(2) || '0.71'}</div>
        </div>
        <div class="metric">
          <div class="metric-label">High Quality Leads</div>
          <div class="metric-value">${analytics.high_quality_leads || '389'}</div>
        </div>
        <div class="metric">
          <div class="metric-label">Model Accuracy</div>
          <div class="metric-value">${analytics.model_accuracy || '76.8%'}</div>
        </div>

        <h2>Score Distribution</h2>
        <table>
          <tr><th>Category</th><th>Percentage</th></tr>
          <tr><td>High (0.75 - 1.0)</td><td>30.4%</td></tr>
          <tr><td>Medium (0.50 - 0.75)</td><td>44.2%</td></tr>
          <tr><td>Low (0.0 - 0.50)</td><td>25.4%</td></tr>
        </table>

        <h2>Property Type Distribution</h2>
        <table>
          <tr><th>Property Type</th><th>Percentage</th></tr>
          <tr><td>Single Family</td><td>42.1%</td></tr>
          <tr><td>Condo</td><td>28.3%</td></tr>
          <tr><td>Townhouse</td><td>19.6%</td></tr>
          <tr><td>Multi-Family</td><td>10.0%</td></tr>
        </table>

        <div class="footer">
          <p>LEGEAI - Lead Generation AI System | Home Insurance Analytics</p>
        </div>
      </body>
      </html>
    `;
  ...
  const sendEmail = async () => {
    ...
    try {
      const response = await fetch('http://localhost:8000/api/home-insurance/analytics/email', {
        method: 'POST',
        headers: {
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-6">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <button
                onClick={() => exportAnalyticsToExcel(analytics, 'home-insurance')}
              ...
              <button
                onClick={() => exportAnalyticsToXML(analytics, 'home-insurance')}
        ...
        <div className="bg-white border-2 border-slate-200 rounded-lg p-4 mb-6 shadow-md">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              ...
              <input
                ...
                placeholder="Search leads, property types..."
          ...
          {searchQuery && (
            <div className="mt-3 text-sm text-slate-600">
              🔍 Searching for: <span className="font-bold text-emerald-700">{searchQuery}</span>
              <button
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
          ...
          <div className="bg-white border-2 border-blue-200 rounded-lg p-4 shadow-md">
            ...
            <p className="text-3xl font-bold text-blue-700 mt-2">
              {analytics?.avg_score?.toFixed(2) || '0.63'}
            </p>
          ...
          <div className="bg-white border-2 border-purple-200 rounded-lg p-4 shadow-md">
            ...
            <p className="text-3xl font-bold text-purple-700 mt-2">
              {analytics?.high_quality_leads || '298'}
            </p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            ...
            <div className="space-y-3">
              <div>
                <div className="flex justify-between mb-1">
                  ...
                  <span className="text-sm font-bold text-emerald-700">23.3%</span>
                </div>
                <div className="w-full bg-slate-200 rounded-full h-3">
                  <div className="bg-emerald-500 h-3 rounded-full" style={{ width: '23.3%' }}></div>
                </div>
              </div>
              <div>
                <div className="flex justify-between mb-1">
                  <span className="text-sm font-medium text-slate-700">Medium (0.50 - 0.75)</span>
                  <span className="text-sm font-bold text-blue-700">48.9%</span>
                </div>
                <div className="w-full bg-slate-200 rounded-full h-3">
                  <div className="bg-blue-500 h-3 rounded-full" style={{ width: '48.9%' }}></div>
                </div>
              </div>
              <div>
                <div className="flex justify-between mb-1">
                  <span className="text-sm font-medium text-slate-700">Low (0.0 - 0.50)</span>
                  <span className="text-sm font-bold text-amber-700">27.8%</span>
                </div>
                <div className="w-full bg-slate-200 rounded-full h-3">
                  <div className="bg-amber-500 h-3 rounded-full" style={{ width: '27.8%' }}></div>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white border-2 border-blue-200 rounded-lg p-6 shadow-lg">
            <h2 className="text-xl font-bold text-slate-900 mb-4">🏠 Property Type Distribution</h2>
            <div className="space-y-3">
              <div>
                <div className="flex justify-between mb-1">
                  <span className="text-sm font-medium text-slate-700">Single Family</span>
                  <span className="text-sm font-bold text-emerald-700">52.4%</span>
                </div>
                <div className="w-full bg-slate-200 rounded-full h-3">
                  <div className="bg-emerald-500 h-3 rounded-full" style={{ width: '52.4%' }}></div>
                </div>
              </div>
              <div>
                <div className="flex justify-between mb-1">
                  <span className="text-sm font-medium text-slate-700">Condo</span>
                  <span className="text-sm font-bold text-blue-700">24.1%</span>
                </div>
                <div className="w-full bg-slate-200 rounded-full h-3">
                  <div className="bg-blue-500 h-3 rounded-full" style={{ width: '24.1%' }}></div>
                </div>
              </div>
              <div>
                <div className="flex justify-between mb-1">
                  <span className="text-sm font-medium text-slate-700">Townhouse</span>
                  <span className="text-sm font-bold text-purple-700">15.3%</span>
                </div>
                <div className="w-full bg-slate-200 rounded-full h-3">
                  <div className="bg-purple-500 h-3 rounded-full" style={{ width: '15.3%' }}></div>
                </div>
              </div>
              <div>
                <div className="flex justify-between mb-1">
                  <span className="text-sm font-medium text-slate-700">Multi-Family</span>
                  <span className="text-sm font-bold text-amber-700">8.2%</span>
                </div>
                <div className="w-full bg-slate-200 rounded-full h-3">
                  <div className="bg-amber-500 h-3 rounded-full" style={{ width: '8.2%' }}></div>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="bg-white border-2 border-purple-200 rounded-lg p-6 shadow-lg mb-6">
          ...
          <div className="space-y-3">
            <div>
              <div className="flex justify-between mb-1">
                <span className="text-sm font-medium text-slate-700">Property Age</span>
                <span className="text-sm font-bold text-purple-700">22.8%</span>
              </div>
              <div className="w-full bg-slate-200 rounded-full h-4">
                <div className="bg-purple-500 h-4 rounded-full" style={{ width: '22.8%' }}></div>
              </div>
            </div>
            <div>
              <div className="flex justify-between mb-1">
                <span className="text-sm font-medium text-slate-700">Location Risk</span>
                <span className="text-sm font-bold text-emerald-700">20.4%</span>
              </div>
              <div className="w-full bg-slate-200 rounded-full h-4">
                <div className="bg-emerald-500 h-4 rounded-full" style={{ width: '20.4%' }}></div>
              </div>
            </div>
            <div>
              <div className="flex justify-between mb-1">
                <span className="text-sm font-medium text-slate-700">Construction Type</span>
                <span className="text-sm font-bold text-blue-700">18.6%</span>
              </div>
              <div className="w-full bg-slate-200 rounded-full h-4">
                <div className="bg-blue-500 h-4 rounded-full" style={{ width: '18.6%' }}></div>
              </div>
            </div>
            <div>
              <div className="flex justify-between mb-1">
                <span className="text-sm font-medium text-slate-700">Square Footage</span>
                <span className="text-sm font-bold text-amber-700">16.1%</span>
              </div>
              <div className="w-full bg-slate-200 rounded-full h-4">
                <div className="bg-amber-500 h-4 rounded-full" style={{ width: '16.1%' }}></div>
              </div>
            </div>
            <div>
              <div className="flex justify-between mb-1">
                <span className="text-sm font-medium text-slate-700">Roof Age</span>
                <span className="text-sm font-bold text-red-700">13.7%</span>
              </div>
              <div className="w-full bg-slate-200 rounded-full h-4">
                <div className="bg-red-500 h-4 rounded-full" style={{ width: '13.7%' }}></div>
              </div>
            </div>
            <div>
              <div className="flex justify-between mb-1">
                <span className="text-sm font-medium text-slate-700">Other Factors</span>
                <span className="text-sm font-bold text-slate-700">8.4%</span>
              </div>
              <div className="w-full bg-slate-200 rounded-full h-4">
                <div className="bg-slate-500 h-4 rounded-full" style={{ width: '8.4%' }}></div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-gradient-to-br from-emerald-50 to-emerald-100 border-2 border-emerald-200 rounded-lg p-6">
            ...
            <p className="text-sm text-slate-700">
              Properties with brick construction show 35% higher conversion rates compared to wood frame homes.
            </p>
          </div>
          <div className="bg-gradient-to-br from-blue-50 to-blue-100 border-2 border-blue-200 rounded-lg p-6">
            <div className="text-3xl mb-3">📈</div>
            <h3 className="font-bold text-blue-900 mb-2">Trend Alert</h3>
            <p className="text-sm text-slate-700">
              Condo insurance inquiries increased by 22% this quarter, driven by urban market growth.
            </p>
          </div>
          <div className="bg-gradient-to-br from-purple-50 to-purple-100 border-2 border-purple-200 rounded-lg p-6">
            ...
            <p className="text-sm text-slate-700">
              Target properties under 15 years old with low location risk for optimal conversion rates.
            </p>
          </div>
        </div>
        ...
        <div className="mt-6">
          ...
          <AnalyticsCharts colorScheme="emerald" />
        </div>
      </div>
    </div>
  );
