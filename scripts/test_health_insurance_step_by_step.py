"""
Step-by-Step Health Insurance Testing
...
# Test lead for health insurance
test_lead = {
    "lead_id": "HEALTH_STEP_TEST_001",
    "age": 32,
    "income": 95000,
    "policy_type": "health",
...
print("\n" + "="*80)
print("🏥 HEALTH INSURANCE - STEP-BY-STEP MODEL TESTING")
print("="*80)
print(f"\n📋 Test Lead:")
...
try:
    response = requests.get(f"{BASE_URL}/health-insurance/health")
...
try:
    response = requests.get(f"{BASE_URL}/health-insurance/model-info")
    if response.status_code == 200:
        result = response.json()
...
try:
    response = requests.post(
        f"{BASE_URL}/health-insurance/compare-models",
        json=test_lead,
...
try:
    response = requests.post(
        f"{BASE_URL}/health-insurance/score-lead",
        json=test_lead,
        headers={"Content-Type": "application/json"}
    )
...
# Summary
print("\n" + "="*80)
print("✅ HEALTH INSURANCE TESTING COMPLETE")
print("="*80)
print("\nAll steps completed. Review the results above.")
print("\nNext: You can test with different leads or compare with other insurance types.")
print("="*80 + "\n")
